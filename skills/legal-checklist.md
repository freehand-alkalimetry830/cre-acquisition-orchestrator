# Legal & Closing Checklist - Multifamily Acquisition
...
- **All legal phase agents** (mandatory): Every agent in the legal phase must read the section(s) relevant to its assignment. Specifically:
  - **PSA reviewer**: Due Diligence Period Legal Items (PSA Review section)
  - **Title-survey reviewer**: Title & Survey section
  - **Estoppel tracker**: Tenant-Related section (Estoppel Certificates and Batching Strategy)
  - **Loan doc reviewer**: Financing Documents section
  - **Insurance coordinator**: Environmental section (Environmental Insurance) and Closing Documents (Insurance requirements)
  - **Transfer doc preparer**: Closing Documents section (Core documents, Deed types, Prorations)
- **Closing coordinator** (mandatory): Read the Timeline / Critical Path section and the Post-Closing Items section in full. These define every deadline, dependency, and post-closing task that must be tracked.

### What to Cross-Reference

- **Checklist items against PSA deadlines**: Every item in this checklist has an associated deadline that derives from the PSA timeline (e.g., due diligence expiration on Day 25-45, financing contingency on Day 30-45, estoppel delivery on Day 20-35). Cross-reference each checklist item's completion status against its PSA deadline. Items that will miss their deadline must be escalated immediately.
- **DD findings against legal contingencies**: Due diligence findings (environmental issues, physical condition defects, title exceptions, zoning non-conformance) map directly to legal contingencies in the PSA. If a DD agent flagged a REC in the Phase I, the legal team must ensure the environmental contingency is not waived until the issue is resolved. Cross-reference every DD finding with the corresponding PSA contingency.
- **Financing terms against loan doc requirements**: The loan commitment letter specifies terms (rate, LTV, DSCR, reserves, prepayment). The loan documents must accurately reflect these terms. Cross-reference every term in the commitment letter against the corresponding provision in the loan agreement, promissory note, and mortgage/deed of trust.

### How to Apply

- **Legal orchestrator assigns checklist sections**: The legal orchestrator divides this checklist among the legal phase agents. Each agent receives the sections relevant to its specialty. The orchestrator tracks completion across all sections using the closing coordinator's timeline.
- **Each agent checks off items as completed**: As each checklist item is addressed, the agent marks it complete and logs a FINDING entry with the result (e.g., "Title commitment reviewed -- 3 exceptions identified, 2 standard, 1 requires investigation"). Items that need attention are flagged with a recommended action.
- **Closing coordinator uses the timeline template**: The Critical Path section provides the day-by-day timeline template. The closing coordinator populates this template with the deal's actual PSA dates and tracks every milestone against its deadline. Any milestone at risk of being missed triggers an escalation to the legal orchestrator.

### Common Mistakes

- **Missing notice deadlines**: The PSA contains strict notice deadlines (e.g., title objection deadline, due diligence waiver date, financing contingency expiration). Missing a notice deadline can result in waiving contingencies by default or forfeiting the earnest money deposit. Calendar every deadline from Day 0 (PSA execution) and set alerts for 5 business days before each.
- **Not cross-referencing DD findings with PSA representations**: The PSA contains seller representations about property condition, litigation, environmental compliance, and tenant matters. If DD agents discover facts that contradict these representations, the buyer has grounds for price adjustment, extended contingencies, or termination. Failing to cross-reference means missing leverage in negotiations.
- **Skipping post-closing items**: Post-closing tasks (tenant notifications, utility transfers, entity registrations, vendor rebidding, tax reassessment monitoring) are legally required in many jurisdictions. Skipping them creates compliance risk, missed revenue, and potential liability. The post-closing checklist must be tracked with the same rigor as the pre-closing items.
